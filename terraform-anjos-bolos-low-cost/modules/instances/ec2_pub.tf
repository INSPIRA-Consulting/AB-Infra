...
resource "aws_instance" "frontend_1a" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo 'Parando RabbitMQ nativo para evitar conflito...'",
      "sudo systemctl stop rabbitmq-server || echo 'RabbitMQ nativo não estava rodando'",
      "sudo systemctl disable rabbitmq-server || echo 'Falha ao desabilitar RabbitMQ nativo'",
